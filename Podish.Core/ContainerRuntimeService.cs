...
public sealed class ContainerRuntimeService
{
    ...
    public async Task<int> RunAsync(ContainerRunRequest request)
    {
        ...
        try
        {
            ...
            {
                Console.Error.WriteLine($"[FiberPod Error] RootFS path not found: {request.RootfsPath}");
        ...
        out string error)
    {
        ...
        foreach (var layer in storedImage.Layers)
        {
            ...
            try
            {
                var entries = System.Text.Json.JsonSerializer.Deserialize<List<LayerIndexEntry>>(File.ReadAllText(layer.IndexPath));
                if (entries == null)
                {
