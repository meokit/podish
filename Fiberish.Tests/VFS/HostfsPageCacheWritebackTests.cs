...
public class HostfsPageCacheWritebackTests
{
    ...
    [Fact]
    public async Task Close_MustNotFlushDirtyHostMappedPages()
    {
        var root = Path.Combine(Path.GetTempPath(), "hostfs-close-no-flush-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
        var hostFile = Path.Combine(root, "data.bin");
        File.WriteAllText(hostFile, "world");
        ...
        try
        {
            ...
            const uint bufAddr = 0x42200000;
            MapUserPage(mm, engine, bufAddr);
            Assert.True(engine.CopyToUser(bufAddr, "XY"u8.ToArray()));

            var writeRc = await CallSys(sm, engine, "SysWrite", (uint)fd, bufAddr, 2);
            Assert.Equal(2, writeRc);
            Assert.NotNull(inode.Mapping);
            Assert.True(inode.Mapping!.IsDirty(0));

            using var flushCalled = new ManualResetEventSlim(false);
            WindowedMappedFilePageBackend.FlushOverrideForTests = (_, _) =>
            {
                flushCalled.Set();
                return true;
            };
            ...
            try
            {
                ...
                Assert.False(flushCalled.IsSet);
            }
            finally
            {
                WindowedMappedFilePageBackend.FlushOverrideForTests = null;
            }
        }
        finally
        {
