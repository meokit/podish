...
public class FallocateSyscallTests
{
    [Fact]
    public async Task Fallocate_ModeZero_GrowsFileToOffsetPlusLength()
    {
        using var env = new TestEnv();
        env.MapUserPage(0x10000);
        env.WriteCString(0x10000, "/fallocate-grow");

        Assert.Equal(0, await env.Call("SysMknodat", LinuxConstants.AT_FDCWD, 0x10000, 0x8000 | 0x1A4));

        int fd = await env.Call("SysOpen", 0x10000, (uint)FileFlags.O_RDWR);
        Assert.True(fd >= 0);

        const long offset = 4096;
        const long length = 250000;
        Assert.Equal(0, await env.Call(
            "SysFallocate",
            (uint)fd,
            0,
            (uint)offset,
        ...
        Assert.Equal(offset + length, await env.Call("SysLseek", (uint)fd, 0, 2));
    ...
    private sealed class TestEnv : IDisposable
    {
        public TestEnv()
        {
            ...
            var sb = tmpfsType.CreateAnonymousFileSystem().ReadSuper(tmpfsType, 0, "fallocate-tmpfs", null);
