...
public class PipeTests
{
    ...
    public async Task Pipe_RegisterWaitHandle_UnreadablePipe_DoesNotSpuriouslyFire()
    {
        ...
        Assert.Equal(1, inode.WriteFromHost(env.Task, wFile, [0x2A], 0));
    ...
    public async Task Pipe_WaitForWrite_SmallAtomicWrite_WaitsForFullPipeBufSpace()
    {
        ...
        Assert.Equal(65535, inode.WriteFromHost(env.Task, wFile, new byte[65535], 0));

        var pending = inode.WaitForWrite(wFile, env.Task!, PipeInode.PipeBuf).AsTask();
        Assert.False(pending.IsCompleted);

        Assert.Equal(1, inode.ReadToHost(env.Task, rFile, new byte[1], 0));
        for (var i = 0; i < 3; i++)
        {
            env.DrainEvents();
            await Task.Delay(1);
        ...
        Assert.Equal(PipeInode.PipeBuf - 1, inode.ReadToHost(env.Task, rFile, new byte[PipeInode.PipeBuf - 1], 0));
