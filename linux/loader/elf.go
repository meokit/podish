...
func Load(filename string, mm *mem.VMAManager, args []string, envs []string) (*LoaderResult, error) {
	...
	entryRVA := uint64(ef.Entry)
	for _, p := range ef.Progs {
		if p.Type == elf.PT_LOAD && entryRVA >= p.Vaddr && entryRVA < (p.Vaddr+p.Memsz) {
	...
	// Calculate initial Brk (End of loaded segments)
	brkAddr := uint32(0)
	for _, prog := range ef.Progs {
		if prog.Type == elf.PT_LOAD {
			end := uint32(prog.Vaddr) + uint32(prog.Memsz) + loadBase
			// Align up to page?
			end = (end + 0xFFF) &^ 0xFFF
			if end > brkAddr {
				brkAddr = end
