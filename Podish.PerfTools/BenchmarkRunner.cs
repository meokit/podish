...
internal static class BenchmarkRunner
{
    ...
    public static int Run(string[] args)
    {
        ...
        if (options.Engine == "jit" && disableSuperopcodesForRun)
            EnsurePodishCliBuild(projectRoot, jitConfiguration, enableSuperOpcodes: false, enableBlockProfile: options.EnableBlockProfile);
        ...
        var selectedCases = options.Cases.Count > 0 ? options.Cases : PerfWorkloadCases.DefaultRunnerCases.ToList();
        ...
        if (options.JitHandlerProfileBlockDump)
        {
            ...
            if (options.EnableBlockProfile)
                Console.WriteLine("[runner] block_profile=enabled");
    ...
    private static RunnerOptions ParseArgs(string[] args)
    {
        var options = new RunnerOptions
        {
            ...
            EnableBlockProfile = HasFlag(args, "--enable-block-profile"),
        ...
        if (cases.Count == 0)
            cases = new List<string>(PerfWorkloadCases.DefaultRunnerCases);
        options.Cases = cases;
        var defaultIterations = cases.Count == 1
            ? PerfWorkloadCases.DefaultRunnerIterations(cases[0])
            : 30000;
        var defaultTimeout = cases.Count == 1
            ? PerfWorkloadCases.DefaultRunnerTimeoutSeconds(cases[0])
            : 1800;
        options.Iterations = GetIntValue(args, "--iterations", defaultIterations);
        options.Timeout = GetIntValue(args, "--timeout", defaultTimeout);
        ...
        if (options.Cases.Any(caseName => !PerfWorkloadCases.IsSupported(caseName)))
            throw new ArgumentException($"--case must be one of: {PerfWorkloadCases.SupportedCasesText()}");
    ...
    private static void PrintSummary(List<SampleResult> results)
    {
        ...
        foreach (var caseName in PerfWorkloadCases.SupportedCases.Where(caseName =>
                     !string.Equals(caseName, PerfWorkloadCases.GccCompile, StringComparison.Ordinal)))
        {
    ...
    {
        return PerfWorkloadCases.BuildGuestScript(caseName, iterations, MarkerBegin, MarkerEnd);
    }
    ...
    private static void EnsurePodishCliBuild(string projectRoot, string configuration, bool enableSuperOpcodes, bool enableBlockProfile)
    {
        var cmd = new[]
        {
            "dotnet",
            ...
            $"-p:EnableBlockProfile={(enableBlockProfile ? "true" : "false")}"
        };
        Console.WriteLine(
    ...
    private sealed class RunnerOptions
    {
        ...
        public bool EnableBlockProfile { get; set; }
