...
`grep_10m` generates an exact 10 MiB file with BusyBox `yes | head`, warms one untimed grep pass, then profiles
repeated BusyBox `grep -F -c needle` scans over that file.
...
BusyBox grep 10 MiB hotspot workflow:
...
- `--rootfs <rootfs dir>`
- `--bench-case compress|compile|gcc_compile|run|grep_10m`
- `--iterations <n>`
- `--time-limit <seconds>`
- `--top <n>`
- `--disasm-top <n>`
...
- `--rootfs <dir>`
- `--results-dir <dir>`
- `--case <compress|compile|gcc_compile|run|grep_10m>`
- `--repeat <n>`
- `--iterations <n>`
- `--timeout <seconds>`
- `--candidate-top <n>`
