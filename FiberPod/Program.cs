...
internal class Program
{
        ...
        string containerId, string image, string containerDir, ContainerLogDriver logDriver, ContainerEventStore eventStore)
    {
        ...
        try
        {
            ...
            try
            {
                var resolvConfPath = GenerateResolvConf(dnsServers, containersDir);
                Logger.LogInformation("Mounting generated DNS configuration at /etc/resolv.conf");
                runtime.Syscalls.MountHostfs(resolvConfPath, "/etc/resolv.conf", readOnly: true);
            }
            catch (Exception ex)
            {
    ...
    private static string GenerateResolvConf(string[] dnsServers, string containersDir)
    {
        var resolvConfPath = Path.Combine(containersDir, $"resolv_{Guid.NewGuid():N}.conf");
        ...
        File.WriteAllText(resolvConfPath, sb.ToString());
