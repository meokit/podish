...
exit 0
    ...
    # Run locker
    fiberpod_args = [fiberpod_exe, "run", "--project", "FiberPod/FiberPod.csproj", "--", "run", "-v", f"{str(tmp_path)}:/scripts", "tests/alpine-rootfs", "/bin/sh", "/scripts/locker.sh"]
    ...
    # Run verifier
    fiberpod_args_2 = [fiberpod_exe, "run", "--project", "FiberPod/FiberPod.csproj", "--", "run", "-v", f"{str(tmp_path)}:/scripts", "tests/alpine-rootfs", "/bin/sh", "/scripts/verifier.sh"]
