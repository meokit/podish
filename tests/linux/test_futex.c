...
int child_func(void* arg) {
    ...
    if (ret < 0) print_str("Child: wake failed\n");
    else print_str("Child: wake sent\n");
...
int main() {
    ...
    // We'll use fork(). But fork() copies memory (COW). Futexes work on private pages if they are effectively same physical (but COW breaks that).
    // WAIT. FUTEX requires shared memory for it to make sense between processes?
    ...
    */
    ...
    if (ret == 0) print_str("Parent: Woken up!\n");
    else if (errno == EWOULDBLOCK) print_str("Parent: EWOULDBLOCK (already changed)\n");
    else print_str("Parent: Other return (maybe error)\n");
    
    if (futex_addr == 1) print_str("Parent: Verified value is 1. SUCCESS.\n");
    else print_str("Parent: Value is not 1. FAIL.\n");
