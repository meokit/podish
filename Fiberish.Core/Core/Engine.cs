...
public class Engine : IDisposable
{
    ...
    internal unsafe int GatherUserBufferSegments(
        ArraySegment<Iovec> iovs,
        int startIovIndex,
        uint startIovOffset,
        bool isWrite,
        int maxBytes,
        Span<ResolvedBufferSegment> segments,
        out int bytesResolved,
        out int nextIovIndex,
        out uint nextIovOffset)
    {
        EnsureAddressSpaceSynchronized();

        bytesResolved = 0;
        nextIovIndex = startIovIndex;
        nextIovOffset = startIovOffset;

        if (maxBytes <= 0 || startIovIndex < 0 || startIovIndex > iovs.Count || segments.Length == 0)
            return 0;

        var segmentCount = 0;
        for (var i = startIovIndex; i < iovs.Count && bytesResolved < maxBytes; i++)
        {
            var iov = iovs[i];
            var iovOffset = i == startIovIndex ? startIovOffset : 0u;
            if (iovOffset > iov.Len)
                break;

            var remainingInIov = iov.Len - iovOffset;
            while (remainingInIov > 0 && bytesResolved < maxBytes)
            {
                var currAddr = iov.BaseAddr + iovOffset;
                var ptr = isWrite ? X86Native.ResolvePtrForWrite(State, currAddr) : X86Native.ResolvePtrForRead(State, currAddr);
                if (ptr == null && TryResolveUserAccessFault(currAddr, isWrite))
                    ptr = isWrite ? X86Native.ResolvePtrForWrite(State, currAddr) : X86Native.ResolvePtrForRead(State, currAddr);

                if (ptr == null)
                {
                    nextIovIndex = i;
                    nextIovOffset = iovOffset;
                    return segmentCount;
                }

                var pageOffset = currAddr & 0xFFF;
                var chunk = Math.Min(
                    Math.Min(maxBytes - bytesResolved, (int)remainingInIov),
                    4096 - (int)pageOffset);

                var resolvedPtr = (IntPtr)ptr;
                if (segmentCount > 0)
                {
                    var previous = segments[segmentCount - 1];
                    if (previous.Pointer + previous.Length == resolvedPtr)
                    {
                        segments[segmentCount - 1] = previous with { Length = previous.Length + chunk };
                    }
                    else
                    {
                        ...
                        segments[segmentCount++] = new ResolvedBufferSegment(resolvedPtr, chunk);
                    }
                }
                else
                {
                ...
                bytesResolved += chunk;
                iovOffset += (uint)chunk;
                remainingInIov -= (uint)chunk;
            }

            nextIovIndex = i;
            nextIovOffset = iovOffset;
            if (nextIovOffset >= iov.Len)
            {
                nextIovIndex = i + 1;
                nextIovOffset = 0;
            }
