...
public class RandomInode : Inode
{
    ...
    public override int WriteFromHost(FiberTask? task, LinuxFile linuxFile, ReadOnlySpan<byte> buffer,
        long offset)
    {
        _ = task;
    ...
    {
        int total = 0;
        for (int i = 0; i < iovs.Count; i++)
        {
            var iov = iovs[i];
            var remaining = iov.Len;
            var addr = iov.BaseAddr;
            while (remaining > 0)
            {
                var ptr = engine.GetPhysicalAddressSafe(addr, true);
                if (ptr == IntPtr.Zero)
                    if (engine.PageFaultResolver != null && engine.PageFaultResolver(addr, true))
                        ptr = engine.GetPhysicalAddressSafe(addr, true);
                if (ptr == IntPtr.Zero)
                    return ValueTask.FromResult(total > 0 ? total : -(int)Errno.EFAULT);

                var pageOff = addr & 0xFFF;
                var chunk = (int)Math.Min(remaining, 4096u - pageOff);

                unsafe { RandomNumberGenerator.Fill(new Span<byte>((void*)ptr, chunk)); }
        ...
        ArraySegment<Iovec> iovs, long offset, int flags)
    {
        ...
        for (int i = 0; i < iovs.Count; i++)
        {
            ...
            while (remaining > 0)
            {
                var ptr = engine.GetPhysicalAddressSafe(addr, false);
                if (ptr == IntPtr.Zero)
                    if (engine.PageFaultResolver != null && engine.PageFaultResolver(addr, false))
                        ptr = engine.GetPhysicalAddressSafe(addr, false);
                if (ptr == IntPtr.Zero)
                    return ValueTask.FromResult(total > 0 ? total : -(int)Errno.EFAULT);
...
public class ZeroInode : Inode
{
    public ZeroInode(SuperBlock sb)
    {
        SuperBlock = sb;
        Type = InodeType.CharDev;
        Mode = 0x1B6; // 666
        ...
        ArraySegment<Iovec> iovs, long offset, int flags)
    {
        ...
        for (int i = 0; i < iovs.Count; i++)
        {
            ...
            while (remaining > 0)
            {
                ...
                unsafe { Unsafe.InitBlockUnaligned((void*)ptr, 0, (uint)chunk); }
