...
{
    public OverlayFileSystem(DeviceNumberManager? devManager = null, MemoryRuntimeContext? memoryContext = null)
    ...
    public override SuperBlock ReadSuper(FileSystemType fsType, int flags, string devName, object? data)
    {
        ...
        var sb = new OverlaySuperBlock(fsType, lowers, upper, DevManager, MemoryContext);
...
public class OverlaySuperBlock : SuperBlock
{
    ...
    public void AddWhiteout(InodeKey dirKey, ReadOnlySpan<byte> name)
    {
        lock (Lock)
        {
            ...
            names.Add(FsName.FromBytes(name));
        }
    }

    public void RemoveWhiteout(InodeKey dirKey, ReadOnlySpan<byte> name)
    {
        lock (Lock)
        {
            if (_whiteouts.TryGetValue(dirKey, out var names))
    ...
    public bool HasWhiteout(InodeKey dirKey, ReadOnlySpan<byte> name)
    {
        lock (Lock)
        {
    ...
    public IReadOnlyCollection<FsName> GetWhiteouts(InodeKey dirKey)
    {
        lock (Lock)
        {
            if (!_whiteouts.TryGetValue(dirKey, out var names)) return Array.Empty<FsName>();
            return [.. names];
        }
    }
...
public sealed class OverlayNodeState
{
    ...
    private readonly FsNameMap<OverlayNodeState> _children = new();
    ...
    public OverlayNodeState GetOrCreateChildState(FsName name, IReadOnlyList<Dentry>? lowers, Dentry? upper)
    {
        lock (SyncRoot)
        {
    ...
    public bool TryRemoveChildState(FsName name, OverlayNodeState expected)
    {
        lock (SyncRoot)
        {
            var removed = _children.TryGetValue(name, out var state) &&
    ...
    public void AttachChildState(FsName name, OverlayNodeState state, IReadOnlyList<Dentry>? lowers, Dentry? upper)
    {
        lock (SyncRoot)
        {
            state.UpdateBackings(lowers, upper);
    ...
    public bool TryGetCachedEntries(out List<DirectoryEntry> entries)
    {
        lock (SyncRoot)
        {
    ...
    public void SetCachedEntries(List<DirectoryEntry> entries)
    {
        lock (SyncRoot)
        {
    ...
    public void InvalidateEntriesCache()
    {
        lock (SyncRoot)
        {
            _cachedEntries = null;
...
public class OverlayInode : MappingBackedInode
{
    ...
    private enum FileBackingTraceOperation
    {
        Open,
        CopyUp,
        Release
    ...
    private enum FileBackingTracePhase
    {
        None,
        Replace,
        Failed,
    ...
    private static string GetFileBackingReason(FileBackingTraceOperation operation,
        FileBackingTracePhase phase = FileBackingTracePhase.None)
    {
        return (operation, phase) switch
        {
            (FileBackingTraceOperation.Open, FileBackingTracePhase.None) => "OverlayInode.Open",
            (FileBackingTraceOperation.Open, FileBackingTracePhase.Replace) => "OverlayInode.Open.replace",
            (FileBackingTraceOperation.Open, FileBackingTracePhase.Failed) => "OverlayInode.Open.failed",
            (FileBackingTraceOperation.CopyUp, FileBackingTracePhase.None) => "OverlayInode.CopyUp",
            (FileBackingTraceOperation.CopyUp, FileBackingTracePhase.Replace) => "OverlayInode.CopyUp.replace",
            (FileBackingTraceOperation.CopyUp, FileBackingTracePhase.Failed) => "OverlayInode.CopyUp.failed",
            (FileBackingTraceOperation.CopyUp, FileBackingTracePhase.Old) => "OverlayInode.CopyUp.old",
            (FileBackingTraceOperation.CopyUp, FileBackingTracePhase.New) => "OverlayInode.CopyUp.new",
            (FileBackingTraceOperation.Release, FileBackingTracePhase.None) => "OverlayInode.Release",
            _ => throw new ArgumentOutOfRangeException(nameof(phase), phase, null)
        };
    }

    private void BindFileBacking(LinuxFile linuxFile, Inode backing, FileBackingTraceOperation operation,
        FileBackingTracePhase phase = FileBackingTracePhase.None)
    {
        var reason = GetFileBackingReason(operation, phase);
        if (_openBackingByFile.TryGetValue(linuxFile, out var existing))
        {
            if (ReferenceEquals(existing, backing))
                return;
            UnbindFileBacking(linuxFile, operation, FileBackingTracePhase.Replace);
        ...
        catch
        {
            ...
            backing.ReleaseRef(refKind, GetFileBackingReason(operation, FileBackingTracePhase.Failed));
            throw;
        }
    }

    private void UnbindFileBacking(LinuxFile linuxFile, FileBackingTraceOperation operation,
        FileBackingTracePhase phase = FileBackingTracePhase.None)
    {
    ...
    private void RebindFileBacking(LinuxFile linuxFile, Inode backing, FileBackingTraceOperation operation)
    {
        if (_openBackingByFile.TryGetValue(linuxFile, out var existing) && ReferenceEquals(existing, backing))
            return;

        UnbindFileBacking(linuxFile, operation, FileBackingTracePhase.Old);
        linuxFile.PrivateData = null;
        BindFileBacking(linuxFile, backing, operation, FileBackingTracePhase.New);
    }

    private void RebindAllFileBackings(Inode backing, FileBackingTraceOperation operation)
    {
        if (_openBackingByFile.Count == 0)
            return;

        foreach (var linuxFile in _openBackingByFile.Keys.ToArray())
            RebindFileBacking(linuxFile, backing, operation);
    }
    ...
    private void InvalidateAliasMappings()
    {
        ...
        {
            alias.UnmapMappingRange(0, long.MaxValue, true);
            alias.InvalidateInodePages2Range(0, uint.MaxValue);
        }
    }
    ...
    private int CopyUpToCurrentParent(FsName currentName, OverlayInode currentParent, LinuxFile? linuxFile)
    {
        lock (_state.SyncRoot)
        {
            if (UpperInode != null)
    ...
    private int CopyUpToUpper(Dentry upperParent, FsName upperName, LinuxFile? linuxFile)
    {
        ...
        if (Type == InodeType.Directory)
        {
            var existing = upperParent.Inode!.Lookup(upperName.Bytes);
            if (existing != null)
            {
        ...
        switch (Type)
        {
            case InodeType.Symlink:
                ...
                byte[]? linkTarget;
                var readlinkRc = lowerInode.Readlink(out linkTarget);
                if (readlinkRc < 0)
                    return readlinkRc;
        ...
        if (copyPayloadRc < 0)
        {
            _ = upperParent.Inode!.Unlink(upperName.Bytes);
            Logging.CreateLogger<OverlayInode>()
                .LogWarning("CopyUp failed for {Name}: rc={Result}", upperName.ToDebugString(), copyPayloadRc);
    ...
    private void PromoteStateBackings(Inode? newBacking, LinuxFile? triggeringFile = null)
    {
        ...
        foreach (var alias in _state.SnapshotAliases())
        {
            alias.RebindAllFileBackings(newBacking, FileBackingTraceOperation.CopyUp);
            ProcessAddressSpaceSync.MigrateOverlayMappings(alias, newBacking);
        }

        if (triggeringFile != null)
            RebindFileBacking(triggeringFile, newBacking, FileBackingTraceOperation.CopyUp);
    }
    ...
    private int EnsureParentUpper(Dentry lowerDentry, out Dentry upperParent)
    {
        ...
        if (parentLower == null ||
            parentLower == lowerDentry ||
            parentLower.Name.IsEmpty ||
            parentLower.Name == "/")
        {
    ...
    public override Dentry? Lookup(ReadOnlySpan<byte> name)
    {
        ...
        if (osb.HasWhiteout(dirKey, name) || osb.WhiteoutCodec.HasEncodedWhiteout(UpperInode, name))
            return null;
        ...
        return new Dentry(childName, inode, parentDentry, SuperBlock);
    }
    ...
    public override int Create(Dentry dentry, int mode, int uid, int gid)
    {
        ...
        var osb = (OverlaySuperBlock)SuperBlock;
        if (!osb.WhiteoutCodec.IsInternalMarkerName(dentry.Name.Bytes))
        {
            osb.WhiteoutCodec.ClearEncodedWhiteout(this, dentry.Name.Bytes);
            osb.RemoveWhiteout(new InodeKey(Dev, Ino), dentry.Name.Bytes);
        }
    ...
    {
        var rc = Readlink(out byte[]? rawTarget);
        if (rc < 0 || rawTarget == null)
        {
            target = null;
            return rc;
        }

        target = FsEncoding.DecodeUtf8Lossy(rawTarget);
        return 0;
    }
    ...
    public override int Unlink(ReadOnlySpan<byte> name)
    {
        ...
        foreach (var alias in Dentries)
            alias.TryUncacheChild(name, "OverlayInode.Unlink.parent-drop-stale", out _);
        InvalidateEntriesCache();
    ...
    private bool DetachRemovedChildState(FsName name, Dentry overlayEntry)
    {
    ...
    public override int Rename(ReadOnlySpan<byte> oldName, Inode newParent, ReadOnlySpan<byte> newName)
    {
        ...
        if (oldName.SequenceEqual("."u8) ||
            oldName.SequenceEqual(".."u8) ||
            newName.SequenceEqual("."u8) ||
            newName.SequenceEqual(".."u8))
            return -(int)Errno.EINVAL;
        if (ReferenceEquals(this, targetParent) && oldName.SequenceEqual(newName))
            return 0;
        ...
        if (!ReferenceEquals(_state, targetParent._state) || !oldName.SequenceEqual(newName))
        {
            _state.TryRemoveChildState(oldNameFs, sourceOverlay._state);
            targetParent._state.AttachChildState(newNameFs, sourceOverlay._state, sourceOverlay.LowerDentries,
        ...
        if (!ReferenceEquals(this, targetParent))
            targetParent.InvalidateEntriesCache();
        ...
        long offset, int flags)
    {
        ...
        return source != null
            ? source.ReadV(engine, file, task, iovs, offset, flags)
        ...
        ReadOnlySpan<byte> buffer, long offset)
    {
        ...
        var rc = source.WriteFromHost(null, linuxFile, buffer, offset);
        if (rc > 0)
    ...
    {
        var copyRc = EnsureWritableBacking(file);
        if (copyRc < 0)
            return ValueTask.FromResult(copyRc);

        var source = ResolveSourceForFile(file);
        if (source == null)
            return ValueTask.FromResult(-(int)Errno.EROFS);

        return WriteVCoreAsync();

        async ValueTask<int> WriteVCoreAsync()
        {
            var rc = await source.WriteV(engine, file, task, iovs, offset, flags);
            if (rc > 0)
                InvalidateAliasMappings();
            return rc;
        }
    ...
    public override void Open(LinuxFile linuxFile)
    {
        ...
        BindFileBacking(linuxFile, source, FileBackingTraceOperation.Open);
    }

    public override void Release(LinuxFile linuxFile)
    {
        _ = Flock(linuxFile, LinuxConstants.LOCK_UN);
        UnbindFileBacking(linuxFile, FileBackingTraceOperation.Release);
    }
    ...
    public override int Truncate(long size)
    {
        if (UpperInode != null)
        {
            var rc = UpperInode.Truncate(size);
            if (rc == 0)
                InvalidateAliasMappings();
            return rc;
        }
        if (LowerInode == null)
        {
            ...
            var rc = detachedBacking?.Truncate(size) ?? -(int)Errno.EROFS;
            if (rc == 0)
                InvalidateAliasMappings();
            return rc;
        }
        ...
        var res = CopyUp(null);
        if (res < 0) return res;
        var truncateRc = UpperInode!.Truncate(size);
        if (truncateRc == 0)
            InvalidateAliasMappings();
        return truncateRc;
    }
    ...
    {
        if (_state.TryGetCachedEntries(out var cachedEntries))
            return cachedEntries;

        var osb = (OverlaySuperBlock)SuperBlock;
        var dirKey = new InodeKey(Dev, Ino);
        var lowerEntrySets = new List<List<DirectoryEntry>>(LowerDentries.Count);
        var capacity = osb.GetWhiteouts(dirKey).Count;
        for (var i = LowerDentries.Count - 1; i >= 0; i--)
        {
            var lowerEntries = LowerDentries[i].Inode?.GetEntries();
            if (lowerEntries == null)
                continue;

            lowerEntrySets.Add(lowerEntries);
            capacity += lowerEntries.Count;
        }

        var upperEntries = UpperInode?.GetEntries();
        capacity += upperEntries?.Count ?? 0;
        var entries = new Dictionary<FsName, DirectoryEntry>(Math.Max(capacity, 4));
        var dirOpaque = osb.IsOpaque(dirKey) || osb.WhiteoutCodec.IsEncodedOpaque(UpperInode);

        // Merge lower layers from bottom to top so higher lower layers override.
        if (!dirOpaque)
            foreach (var lowerEntries in lowerEntrySets)
                foreach (var e in lowerEntries)
                    entries[e.Name] = e;

        if (upperEntries != null)
            foreach (var e in upperEntries)
            {
        ...
        List<DirectoryEntry> visibleEntries = [.. entries.Values];
        _state.SetCachedEntries(visibleEntries);
        return visibleEntries;
    }

    private Dentry? LookupInAnyLower(ReadOnlySpan<byte> name)
    {
        foreach (var lower in LowerDentries)
        {
            var candidate = lower.Inode?.Lookup(name);
    ...
    {
        return LookupInAnyLower(FsEncoding.EncodeUtf8(name));
    }
...
{
    bool IsInternalMarkerName(ReadOnlySpan<byte> name);
    bool HasEncodedWhiteout(Inode? upperDir, ReadOnlySpan<byte> name);
    bool IsEncodedOpaque(Inode? upperDir);
    bool IsEncodedOpaqueEntry(DirectoryEntry entry);
    bool TryDecodeEncodedWhiteout(DirectoryEntry entry, Inode? inode, out FsName targetName);
    bool IsWhiteoutInode(Inode? inode);
    bool TryCreateEncodedWhiteout(OverlayInode dir, ReadOnlySpan<byte> name);
    void ClearEncodedWhiteout(OverlayInode dir, ReadOnlySpan<byte> name);
}

public sealed class LogicalWhiteoutCodec : IOverlayWhiteoutCodec
{
    public bool IsInternalMarkerName(ReadOnlySpan<byte> name)
    {
        return false;
    }
    ...
    public bool TryDecodeEncodedWhiteout(DirectoryEntry entry, Inode? inode, out FsName targetName)
    {
        targetName = FsName.Empty;
        return false;
    }
    ...
    public bool TryCreateEncodedWhiteout(OverlayInode dir, ReadOnlySpan<byte> name)
    {
        return false;
    }

    public void ClearEncodedWhiteout(OverlayInode dir, ReadOnlySpan<byte> name)
    {
    }
}
...
public sealed class HybridWhiteoutCodec : IOverlayWhiteoutCodec
{
    ...
    public bool IsInternalMarkerName(ReadOnlySpan<byte> name)
    {
        return name.SequenceEqual(OpaqueMarkerBytes) || name.StartsWith(WhiteoutPrefixBytes);
    }
    ...
    {
        if (upperDir == null) return false;
        return upperDir.Lookup(OpaqueMarkerBytes) != null;
    }

    public bool IsEncodedOpaqueEntry(DirectoryEntry entry)
    {
        return entry.Name.Bytes.SequenceEqual(OpaqueMarkerBytes);
    }

    public bool TryDecodeEncodedWhiteout(DirectoryEntry entry, Inode? inode, out FsName targetName)
    {
        if (entry.Name.Bytes.StartsWith(WhiteoutPrefixBytes) && !entry.Name.Bytes.SequenceEqual(OpaqueMarkerBytes))
        {
            var suffix = entry.Name.Bytes[WhiteoutPrefixBytes.Length..];
            if (suffix.IsEmpty)
            {
                targetName = FsName.Empty;
                return false;
            ...
            targetName = FsName.FromBytes(suffix);
            return true;
        }
    ...
    public bool TryCreateEncodedWhiteout(OverlayInode dir, ReadOnlySpan<byte> name)
    {
        ...
        var encodedName = dir.UpperInode is SilkInode
            ? FsName.FromOwnedBytes(BuildEncodedWhiteoutName(name))
            : FsName.FromBytes(name);
        if (dir.UpperInode.Lookup(encodedName.Bytes) != null) return false;
