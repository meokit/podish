...
public class Hostfs : FileSystem
{
    ...
    public override SuperBlock ReadSuper(FileSystemType fsType, int flags, string devName, object? data)
    {
        ...
        var sb = new HostSuperBlock(fsType, devName, opts, DevManager,
            MemoryContext); // devName is the root path on host
...
public partial class HostInode : MappingBackedInode, IHostMappedCacheDropper
{
    ...
    private int _projectedMetadataQueryGid;
    private int _projectedMetadataQueryUid;
    ...
    private static HostOpenFileState? GetOpenState(LinuxFile? linuxFile)
    {
        return linuxFile?.PrivateData as HostOpenFileState;
    }

    private static SafeFileHandle? GetOpenHandleFromFile(LinuxFile? linuxFile)
    {
    ...
    {
        var path = ResolveHostPath(linuxFile);
        lock (_mappedCacheLock)
        {
            _mappedPageCache ??= new MappedFilePageCache(path, SuperBlock.MemoryContext.HostMemoryMapGeometry);
            _mappedPageCache.UpdatePath(path);
            return _mappedPageCache;
        }
    }
    ...
    private void InvalidateProjectedMetadataCache()
    {
        lock (Lock)
        {
            _hasProjectedMetadataCache = false;
        }
    }
    ...
    public void RefreshProjectedMetadata(int queryUid, int queryGid)
    {
        lock (Lock)
        {
            if (_hasProjectedMetadataCache &&
                _projectedMetadataQueryUid == queryUid &&
                _projectedMetadataQueryGid == queryGid)
                return;
        }
        ...
        lock (Lock)
        {
            _projectedMetadataQueryUid = queryUid;
            _projectedMetadataQueryGid = queryGid;
            _hasProjectedMetadataCache = true;
        }
    }

    public int SetProjectedOwnership(int uid, int gid)
    {
        var rc = HostfsOwnershipMapper.SetGuestOwnership(this, uid, gid);
        if (rc == 0)
            InvalidateProjectedMetadataCache();
        return rc;
    }
    ...
    {
        var rc = HostfsOwnershipMapper.SetGuestMode(this, mode);
        if (rc == 0)
            InvalidateProjectedMetadataCache();
        return rc;
    }
    ...
    public override void Release(LinuxFile linuxFile)
    {
        ...
        switch (linuxFile.PrivateData)
        {
            case HostOpenFileState state:
    ...
    protected override void FlushHandleToDisk(SafeFileHandle handle)
    {
        FlushHandleToDiskInternal(handle);
    }
    ...
    public override int ReadToHost(FiberTask? task, LinuxFile file, Span<byte> buffer,
        long offset)
    {
        _ = task;
        return ReadWithPageCache(file, buffer, offset);
    }

    protected internal override int BackendWrite(LinuxFile? linuxFile, ReadOnlySpan<byte> buffer, long offset)
    {
        return BackendWriteCore(linuxFile, buffer, offset, true);
    }

    private int BackendWriteAtOffset(LinuxFile? linuxFile, ReadOnlySpan<byte> buffer, long offset)
    {
        return BackendWriteCore(linuxFile, buffer, offset, false);
    }
    ...
    private int BackendWriteCore(LinuxFile? linuxFile, ReadOnlySpan<byte> buffer, long offset, bool honorAppend)
    {
        ...
        if (!OperatingSystem.IsWindows() && GetOpenState(linuxFile)?.DirectHandle is { } directHandle)
        {
            ...
            if ((long)logicalEnd > cachedSizeBeforeWrite)
                ObserveBackingSizeChange((long)logicalEnd, default);
    ...
    public override int WriteFromHost(FiberTask? task, LinuxFile file,
        ReadOnlySpan<byte> buffer, long offset)
    {
        _ = task;
        var writeThroughPageCacheOnly = SuperBlock.MemoryContext.HostMemoryMapGeometry.SupportsMappedFileBackend;
        var originalSize = writeThroughPageCacheOnly ? 0 : (long)Size;
        var prepareRc = PrepareHostMappedWrite(file, buffer.Length, offset);
        if (prepareRc < 0)
            return prepareRc;
    ...
    protected override int PrepareHostMappedWrite(LinuxFile? linuxFile, long bufferLength, long offset)
    {
        ...
        var cachedBackingLength = CachedSizeForShrinkCoordinator;
        if (writeEnd <= cachedBackingLength)
        {
            return 0;
        }
        ...
        try
        {
            ...
            ObserveBackingSizeChange(preparedLength, default);
            return 0;
        }
        catch (Exception ex)
        {
    ...
    {
        if (Type != InodeType.File || pageCount <= 0)
            return 0;
        ...
        if (!TryGetMappedBackingFileLength(linuxFile, out var mappedLength))
            mappedLength = Math.Max(0, fileSize);

        lock (_mappedCacheLock)
        {
            var path = linuxFile != null ? ResolveHostPath(linuxFile) : ResolveHostPath();
            _mappedPageCache ??= new MappedFilePageCache(path, SuperBlock.MemoryContext.HostMemoryMapGeometry);
            _mappedPageCache.UpdatePath(path);
            return _mappedPageCache.TryAcquirePageLeases((long)startPageIndex, pageCount, mappedLength, writable,
    ...
    {
        // Host-mapped windows are written back in bulk by FlushMappedWindowsToBacking
        // (called from FlushWritebackToDurable). There is no per-page writeback path
        // because the host OS manages dirty state via mmap.
        _ = linuxFile;
        _ = mapping;
        _ = record;
        return 0;
        ...
        bool writable, out BackingPageHandle backingPageHandle)
    {
        ...
        var pageCache = linuxFile != null ? GetOrCreateMappedPageCache(linuxFile) : null;
        lock (_mappedCacheLock)
        {
            var cache = pageCache;
            if (cache == null)
            {
                ...
                cache = _mappedPageCache;
            }

            if (!cache.TryAcquirePageLease(
                    pageIndex,
                    mappedLength,
                    writable,
                    out var pointer,
    ...
    {
        if (mode == PageWritebackMode.Durable && FlushWritebackToDurable(linuxFile) != 0)
            return false;
