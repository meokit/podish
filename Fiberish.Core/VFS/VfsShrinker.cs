...
public static class VfsShrinker
{
    public static VfsShrinkStats Shrink(SyscallManager? syscallManager, VfsShrinkMode mode)
    {
        ...
        if ((mode & VfsShrinkMode.PageCache) != 0)
        {
            foreach (var sb in superblocks)
                FlushReclaimableDirtyInodes(sb);
    ...
    internal static long EvictUnusedInodes(SuperBlock sb)
    {
        ...
        foreach (var inode in candidates)
        {
            ...
            if (inode.TryEvictCache(InodeLifecycleTraceOperation.VfsShrinkerEvictUnusedInodes))
                evicted++;
        }
    ...
    private static void FlushReclaimableDirtyInodes(SuperBlock sb)
    {
        List<Inode> inodes;
        lock (sb.Lock)
        {
        ...
        foreach (var inode in inodes)
        {
            switch (inode)
            {
                case HostInode hostInode:
                    _ = hostInode.FlushDirtyDataIfNeeded(null);
                    break;
                case SilkInode silkInode:
                    _ = silkInode.FlushDirtyDataIfNeeded(null);
                    break;
            }
        }
    }
