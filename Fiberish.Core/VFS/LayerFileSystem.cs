...
{
    private readonly Dictionary<string, List<LayerDirectoryChild>> _children = new(StringComparer.Ordinal);
    private readonly Dictionary<string, Dictionary<string, string>> _childPathsByName = new(StringComparer.Ordinal);
    ...
    public void AddEntry(LayerIndexEntry entry)
    {
        ...
        var encodedSymlinkTarget = entry.Type == InodeType.Symlink
            ? EncodeUtf8Strict(entry.SymlinkTarget, nameof(entry.SymlinkTarget))
            : null;
        var childName = normalizedPath == "/"
            ? FsName.Empty
            : FsName.FromOwnedBytes(EncodeUtf8Strict(GetBaseName(normalizedPath), nameof(entry.Path)));
        var normalizedEntry = entry with
        {
            Path = normalizedPath,
            Name = childName,
            EncodedSymlinkTarget = encodedSymlinkTarget
        };
        _entries[normalizedPath] = normalizedEntry;

        if (normalizedEntry.Type == InodeType.Directory)
        {
            _childPathsByName.TryAdd(normalizedPath, new Dictionary<string, string>(StringComparer.Ordinal));
            _children.TryAdd(normalizedPath, []);
        ...
        var childNameString = GetBaseName(normalizedPath);
        _childPathsByName[parentPath][childNameString] = normalizedPath;
        _childrenByBytes[parentPath].Set(childName, normalizedPath);

        var parentChildren = _children[parentPath];
        var child = new LayerDirectoryChild(childName, normalizedPath);
        for (var i = 0; i < parentChildren.Count; i++)
            if (parentChildren[i].Name == childName)
            {
                parentChildren[i] = child;
                return;
            }
    ...
    public bool TryGetChildPath(string parentPath, string name, out string childPath)
    {
        ...
        if (!_childPathsByName.TryGetValue(p, out var map)) return false;
    ...
    public IReadOnlyList<LayerDirectoryChild> GetChildren(string parentPath)
    {
        var p = NormalizePath(parentPath);
        if (!_children.TryGetValue(p, out var children)) return Array.Empty<LayerDirectoryChild>();
        return children;
    }
    ...
    private void EnsureDirectory(string path)
    {
        ...
        if (_entries.TryGetValue(p, out var existing))
        {
            ...
            _childPathsByName.TryAdd(p, new Dictionary<string, string>(StringComparer.Ordinal));
            _children.TryAdd(p, []);
    ...
    private static byte[] EncodeUtf8Strict(string value, string paramName)
    {
        try
        {
            return FsEncoding.EncodeUtf8Strict(value);
        }
        catch (EncoderFallbackException ex)
        {
            throw new ArgumentException("Layerfs adapter only accepts UTF-8 representable strings.", paramName, ex);
        }
    }
}

public readonly record struct LayerDirectoryChild(FsName Name, string Path);
...
public sealed record LayerIndexEntry(
    ...
    long DataOffset = -1,
    string BlobDigest = "",
    [property: JsonIgnore] FsName Name = default,
    byte[]? EncodedSymlinkTarget = null);
...
public class LayerInode : MappingBackedInode
{
    ...
    public override Dentry? Lookup(string name)
    {
        ...
        var childEntry = sb.Index.Entries[childPath];
        return new Dentry(childEntry.Name, sb.GetOrCreateInode(childPath), parentDentry, SuperBlock);
    }
    ...
    public override Dentry? Lookup(ReadOnlySpan<byte> name)
    {
        ...
        var parentDentry = Dentries.Count > 0 ? Dentries[0] : null;
        return new Dentry(sb.Index.Entries[childPath].Name, sb.GetOrCreateInode(childPath), parentDentry, SuperBlock);
    }
    ...
    private static int ComputeInitialLinkCount(LayerSuperBlock sb, string path, LayerIndexEntry entry)
    {
        ...
        var childDirCount = 0;
        foreach (var child in sb.Index.GetChildren(path))
        {
            if (!sb.Index.TryGetEntry(child.Path, out var childEntry)) continue;
    ...
    public override int Readlink(out byte[]? target)
    {
        ...
        target = _entry.EncodedSymlinkTarget;
        return 0;
    }
    ...
    public override List<DirectoryEntry> GetEntries()
    {
        ...
        var sb = (LayerSuperBlock)SuperBlock;
        var children = sb.Index.GetChildren(_path);
        var entries = new List<DirectoryEntry>(children.Count);
        foreach (var child in children)
        {
            ...
            {
                Name = child.Name,
                Ino = sb.GetStableIno(child.Path),
