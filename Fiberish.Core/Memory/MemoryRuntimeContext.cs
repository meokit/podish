...
public sealed class MemoryRuntimeContext : IDisposable
{
    ...
    internal byte[] CreateExecRandomBytes(int length, ReadOnlySpan<byte> tagRaw)
    {
        if (length <= 0)
            throw new ArgumentOutOfRangeException(nameof(length));

        var buffer = new byte[length];
        FillExecRandomBytes(buffer, tagRaw);
        return buffer;
    }
    ...
    internal void FillExecRandomBytes(Span<byte> buffer, ReadOnlySpan<byte> tagRaw)
    {
        if (buffer.Length == 0)
            return;
        ...
        var sequence = (ulong)Interlocked.Increment(ref _aslrSequence);
        var tagBytes = tagRaw.IsEmpty ? Array.Empty<byte>() : tagRaw.ToArray();
        Span<byte> header = stackalloc byte[24];
        BinaryPrimitives.WriteUInt64LittleEndian(header[..8], seed);
        BinaryPrimitives.WriteUInt64LittleEndian(header[8..16], sequence);
...
internal sealed class ZeroInode : MappingBackedInode, IDisposable
{
    ...
    internal bool IsHostReadOnlyProtected { get; private set; }
        ...
        lock (_zeroPageGate)
        {
            if (_sharedZeroPagePtr == IntPtr.Zero)
            {
                ...
                try
                {
                    ...
                    var zeroPagePtr = reservation.BasePtr;
                    ...
                    IsHostReadOnlyProtected = hostProtected;
                }
                catch
                {
