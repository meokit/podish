...
public sealed class AddressSpace
{
    ...
    internal int CountLeadingAbsentPages(uint startPageIndex, uint endPageIndexInclusive)
    {
        return Pages.CountLeadingAbsentPages(startPageIndex, endPageIndexInclusive);
    }
    ...
    internal void InstallHostPageRecordsIfAbsentBatch(ReadOnlySpan<InodePageRecord?> records,
        HostPageKind hostPageKind, MappingBackedInode releaseOwner, Span<IntPtr> finalPointers, Span<bool> inserted)
    {
        Pages.InstallHostPageRecordsIfAbsentBatch(records, hostPageKind, releaseOwner, finalPointers, inserted);
    }
    ...
    {
        Pages.MarkDirtyRange(startPageIndex, endPageIndexInclusive);
    }
    ...
    internal int RemovePagesInRange(uint startPageIndex, uint endPageIndex,
    ...
    internal void CollectOwnerRmapHits(uint pageIndex, IntPtr hostPagePtr, HostPageRef hostPageRef,
        List<RmapHit> output)
    {
        lock (_rmapLock)
        {
    ...
    internal bool VisitOwnerRmapRefs<TState>(uint pageIndex, IntPtr hostPagePtr, HostPageRef hostPageRef,
        ref TState state,
        ...
        TbCohWorkSet? tbCohWorkSet = null)
    {
        ...
        var changed = MemoryContext.HostPages.AddOrUpdateRmapRef(hostPagePtr, _pageKind, attachment.Mm, attachment.Vma,
    ...
    private void RemoveDirectRefLocked(RmapAttachment attachment, uint pageIndex, IntPtr hostPagePtr,
        TbCohWorkSet? tbCohWorkSet = null)
    {
        var changed = MemoryContext.HostPages.RemoveRmapRef(hostPagePtr, attachment.Mm, attachment.Vma,
...
public sealed class AnonVma
{
    ...
    public AnonVma(MemoryRuntimeContext memoryContext, AnonVma? parent = null, int initialPageCapacity = 0)
    {
        _memoryContext = memoryContext;
        Parent = parent;
        Root = parent?.Root ?? this;
        Pages = new OwnerPageSlots(memoryContext, CreateOwnerBinding, OnPageBindingChanged,
            initialCapacity: initialPageCapacity);
    }
    ...
    {
        var clone = new AnonVma(_memoryContext, this, Pages.PageCount);
        Pages.CloneInto(clone.Pages, HostPageKind.Anon);
        return clone;
    }
