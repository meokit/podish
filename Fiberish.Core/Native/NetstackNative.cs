...
internal static partial class NetstackNative
{
    ...
    internal static partial int TcpStreamConnect(ulong netnsHandle, ulong socketHandle, uint remoteIpv4Be, ushort remotePort);

    [LibraryImport(LibName, EntryPoint = "fiber_tcp_listener_accept")]
    internal static partial int TcpListenerAccept(ulong netnsHandle, ulong socketHandle, out ulong acceptedSocketHandle);

    [LibraryImport(LibName, EntryPoint = "fiber_tcp_stream_send")]
    internal static unsafe partial int TcpStreamSend(ulong netnsHandle, ulong socketHandle, byte* data, nuint len, out nuint written);

    [LibraryImport(LibName, EntryPoint = "fiber_tcp_stream_recv")]
    internal static unsafe partial int TcpStreamRecv(ulong netnsHandle, ulong socketHandle, byte* buffer, nuint len, out nuint read);
    ...
    internal static partial int TcpStreamGetLocalEndpoint(ulong netnsHandle, ulong socketHandle, out uint ipv4Be, out ushort port);
