...
public class PathWalker
{
    ...
    {
        var effectiveLength = nd.PathLength;
        while (effectiveLength > nd.PathPosition && nd.PathBytes[effectiveLength - 1] == SlashByte)
            effectiveLength--;

        while (nd.PathPosition < effectiveLength)
        {
            while (nd.PathPosition < nd.PathLength && nd.PathBytes[nd.PathPosition] == SlashByte)
                nd.PathPosition++;

            if (nd.PathPosition >= effectiveLength)
                break;
            ...
            var isLast = nd.PathPosition >= effectiveLength;
    ...
    public bool WalkComponent(NameData nd, ReadOnlySpan<byte> name, bool isLast)
    {
        ...
        if (next == null)
        {
            ...
            if (next == null)
            {
                var lookupError = current.Inode.ConsumeLookupFailureError();
