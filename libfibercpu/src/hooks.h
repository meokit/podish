...
class HookManager {
public:
    void set_interrupt_hook(uint8_t vector, InterruptHook handler) { handlers_[vector] = handler; }

    // Trigger an interrupt. Returns true if handled, false if emulator should
    // fault.
    ...
    bool on_invalid_opcode(EmuState* state) { return on_interrupt(state, 6); }

    // Decode Fault -> #UD - Vector 6
    bool on_decode_fault(EmuState* state) { return on_interrupt(state, 6); }
