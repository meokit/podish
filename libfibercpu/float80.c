...
void f80_set_rounding_mode(enum f80_rounding_mode mode) {
    switch (mode) {
        case round_to_nearest: softfloat_roundingMode = softfloat_round_near_even; break;
...
float80 f80_from_int(int64_t i) {
    return from_soft(i64_to_extF80(i));
}
 
int64_t f80_to_int(float80 f) {
    return extF80_to_i64(to_soft(f), softfloat_roundingMode, true);
}
...
#else
float80 f80_from_int(int64_t i) {
    return f80_from_double((double)i);
}

int64_t f80_to_int(float80 f) {
    double d = f80_to_double(f);
    switch (g_RoundingMode) {
        case round_to_nearest: return (int64_t)rint(d);
        case round_down:       return (int64_t)floor(d);
        case round_up:         return (int64_t)ceil(d);
        case round_chop:       return (int64_t)trunc(d);
        default:               return (int64_t)d;
    }
}
...
bool f80_isnan(float80 f) {
    return (f.signExp & 0x7FFF) == 0x7FFF && (f.signif & 0x7FFFFFFFFFFFFFFFUL) != 0;
}
...
float80 f80_sqrt(float80 f) {
    return from_soft(extF80_sqrt(to_soft(f)));
}

float80 f80_add(float80 a, float80 b) {
    return from_soft(extF80_add(to_soft(a), to_soft(b)));
}

float80 f80_sub(float80 a, float80 b) {
    return from_soft(extF80_sub(to_soft(a), to_soft(b)));
}

float80 f80_mul(float80 a, float80 b) {
    return from_soft(extF80_mul(to_soft(a), to_soft(b)));
}

float80 f80_div(float80 a, float80 b) {
    return from_soft(extF80_div(to_soft(a), to_soft(b)));
}
...
bool f80_lt(float80 a, float80 b) {
    return extF80_lt(to_soft(a), to_soft(b));
}
 
bool f80_eq(float80 a, float80 b) {
    return extF80_eq(to_soft(a), to_soft(b));
}
#else
...
bool f80_uncomparable(float80 a, float80 b) {
    return f80_isnan(a) || f80_isnan(b);
}
...
float80 f80_round(float80 f) {
    ...
    double d = f80_to_double(f);
    switch (g_RoundingMode) {
        case round_to_nearest: return f80_from_double(rint(d));
        case round_down:       return f80_from_double(floor(d));
        case round_up:         return f80_from_double(ceil(d));
        case round_chop:       return f80_from_double(trunc(d));
        default:               return f80_from_double(d);
    }
#endif
}
...
float80 f80_scale(float80 x, int scale) {
    ...
    if (scale > 16383) return (x.signExp & 0x8000) ? f80_neg(F80_INF) : F80_INF;
    if (scale < -16382) return (x.signExp & 0x8000) ? f80_neg(F80_ZERO) : F80_ZERO;
...
float80 f80_sin(float80 x) {
    return f80_from_double(sin(f80_to_double(x)));
}

float80 f80_cos(float80 x) {
    return f80_from_double(cos(f80_to_double(x)));
}

float80 f80_log2(float80 x) {
    return f80_from_double(log2(f80_to_double(x)));
}
