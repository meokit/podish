...
struct uint128 softfloat_commonNaNToExtF80UI(const struct commonNaN* aPtr) {
    ...
    // extFloat80 NaNs require both the explicit integer bit (bit 63) and the
    // quiet-NaN bit (bit 62). If we only restore bit 63, a binary64 qNaN with
    // no payload degenerates into an infinity-pattern extFloat80 value.
    z.v0 = aPtr->v64 | UINT64_C(0xC000000000000000);
    return z;
}
...
*----------------------------------------------------------------------------*/
struct uint128 softfloat_propagateNaNExtF80UI(uint_fast16_t uiA64, uint_fast64_t uiA0, uint_fast16_t uiB64,
                                              uint_fast64_t uiB0) {
