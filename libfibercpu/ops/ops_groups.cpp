...
static FORCE_INLINE void OpGroup1_EbIb(EmuState* state, DecodedOp* op) {
    // 80: Arith r/m8, imm8
...
static FORCE_INLINE void OpGroup1_EvIz(EmuState* state, DecodedOp* op) {
...
static FORCE_INLINE void OpGroup4_Eb(EmuState* state, DecodedOp* op) {
    // FE: Group 4 (Byte)
...
static FORCE_INLINE void OpGroup9(EmuState* state, DecodedOp* op) {
...
static FORCE_INLINE void OpXadd_Rm_R(EmuState* state, DecodedOp* op) {
    ...
    uint32_t dest_val = 0;
    if (width == 1)
        dest_val = ReadModRM8(state, op);
    else if (width == 2)
        dest_val = ReadModRM16(state, op);
    else
        dest_val = ReadModRM32(state, op);
...
static FORCE_INLINE void OpCdq(EmuState* state, DecodedOp* op) {
    // 99: CDQ
    uint32_t eax = GetReg(state, EAX);
...
static FORCE_INLINE void OpCwde(EmuState* state, DecodedOp* op) {
