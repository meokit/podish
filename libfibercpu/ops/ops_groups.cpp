...
static FORCE_INLINE LogicFlow OpGroup1_EbIb_Internal(EmuState* state, DecodedOp* op, mem::MicroTLB* utlb) {
    // 80: Arith r/m8, imm8
...
static FORCE_INLINE LogicFlow OpGroup1_Ev_T_Internal(EmuState* state, DecodedOp* op, mem::MicroTLB* utlb) {
...
template <bool UpdateFlags, uint8_t FixedSubOp = 0xFF>
static FORCE_INLINE LogicFlow OpGroup3_Eb_Internal(EmuState* state, DecodedOp* op, mem::MicroTLB* utlb) {
    // F6
...
static FORCE_INLINE LogicFlow OpGroup3_Ev_T_Internal(EmuState* state, DecodedOp* op, mem::MicroTLB* utlb) {
    // F7
    T val;
    if constexpr (sizeof(T) == 2) {
        auto res = ReadModRM<uint16_t, OpOnTLBMiss::Restart>(state, op, utlb);
...
template <bool UpdateFlags, uint8_t FixedSubOp = 0xFF>
static FORCE_INLINE LogicFlow OpGroup4_Eb_Internal(EmuState* state, DecodedOp* op, mem::MicroTLB* utlb) {
    // FE
    auto val_res = ReadModRM<uint8_t, OpOnTLBMiss::Restart>(state, op, utlb);
    if (!val_res) return LogicFlow::RestartMemoryOp;
    uint8_t val = *val_res;
...
template <typename T, bool UpdateFlags, uint8_t FixedSubOp = 0xFF>
static FORCE_INLINE LogicFlow OpGroup5_Ev_T_Internal(EmuState* state, DecodedOp* op, mem::MicroTLB* utlb) {
    // FF
    // RMW Instructions!
...
template <typename T>
static FORCE_INLINE LogicFlow OpXadd_T_Internal(EmuState* state, DecodedOp* op, mem::MicroTLB* utlb) {
