...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        File.WriteAllText(outputPath, RenderCoreProtocols(protocols, repoRoot));
    ...
    private static string RenderCoreProtocols(IReadOnlyList<ProtocolModel> protocols, string repoRoot)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// Generated by Podish.Wayland.Generator");
        foreach (ProtocolModel protocol in protocols)
            sb.AppendLine($"// Source protocol: {protocol.Name} ({SanitizePathForComment(repoRoot, protocol.SourcePath)})");
    ...
    private static string SanitizePathForComment(string repoRoot, string path)
    {
        string fullPath = Path.GetFullPath(path);
        string fullRepoRoot = Path.GetFullPath(repoRoot);
        if (fullPath.StartsWith(fullRepoRoot, StringComparison.Ordinal))
